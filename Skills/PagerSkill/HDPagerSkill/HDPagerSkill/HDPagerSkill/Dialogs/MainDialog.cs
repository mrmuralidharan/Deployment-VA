...
namespace HDPagerSkill.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly PagerEscalationPolicyDiaolog _hdpagerescdialog;
        private readonly PagerCreateIncidentDiaolog _hdpagercreateincidentsdialog;
        private readonly PagerPolicyMembersDiaolog _hdpagermembersdialog;
        private readonly TestDiaolog testdialog;
            ...
            : base(nameof(MainDialog))
        {
            ...
            testdialog = serviceProvider.GetService<TestDiaolog>();
            _sampleAction = serviceProvider.GetService<SampleAction>();
            _hdpagerescdialog = serviceProvider.GetService<PagerEscalationPolicyDiaolog>();
            _hdpagermembersdialog = serviceProvider.GetService<PagerPolicyMembersDiaolog>();
            _hdpagercreateincidentsdialog = serviceProvider.GetService<PagerCreateIncidentDiaolog>();
            AddDialog(_hdpagerescdialog);
            AddDialog(_hdpagermembersdialog);
            AddDialog(_hdpagercreateincidentsdialog);
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["HDPagerSkill"].RecognizeAsync<HdPagerSkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("HDPagerSkill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<HdPagerSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case HdPagerSkillLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id, cancellationToken: cancellationToken);
                            }
                        case HdPagerSkillLuis.Intent.Test:
                            {
                                return await stepContext.BeginDialogAsync(testdialog.Id, cancellationToken: cancellationToken);
                            }
                        case HdPagerSkillLuis.Intent.EscalationPolicy:
                            {
                                return await stepContext.BeginDialogAsync(_hdpagerescdialog.Id, cancellationToken: cancellationToken);
                            }
                        case HdPagerSkillLuis.Intent.CreatePolicy:
                            {
                                return await stepContext.BeginDialogAsync(_hdpagercreateincidentsdialog.Id, cancellationToken: cancellationToken);
                            }
                        case HdPagerSkillLuis.Intent.MembersDisplay:
                            {
                                return await stepContext.BeginDialogAsync(_hdpagermembersdialog.Id, cancellationToken: cancellationToken);
                            }

                        case HdPagerSkillLuis.Intent.None:
                        default:
                            {
