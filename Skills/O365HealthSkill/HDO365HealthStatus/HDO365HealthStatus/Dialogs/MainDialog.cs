...
using HDO365HealthStatus.Extensions;
...
namespace HDO365HealthStatus.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly HealthStatusDialog _healthStatusDialog;
        //private readonly IncidentStatusDialog _incidentStatusDialog;
            ...
            : base(nameof(MainDialog))
        {
            ...
            _healthStatusDialog = serviceProvider.GetService<HealthStatusDialog>();
            
            AddDialog(_sampleDialog);
            AddDialog(_sampleAction);
            AddDialog(_healthStatusDialog);
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["HDO365HealthStatus"].RecognizeAsync<Hdo365HealthStatusLuis>(innerDc.Context, cancellationToken);
        ...
        protected override async Task<DialogTurnResult> OnContinueDialogAsync(DialogContext innerDc, CancellationToken cancellationToken = default)
        {
            ...
            //else
            //{
            //    //r activity = turnContext.Activity;
            ...
            //        {
            //            var state = await StateAccessor.GetAsync(innerDc.Context, () => new SkillState(), cancellationToken: cancellationToken);
            //            state.adaptiveCardResponse = value.ToString();
            //            await innerDc.Context.SendActivityAsync(new Activity(type: ActivityTypes.Trace, text: state.adaptiveCardResponse), cancellationToken);
            //        }
            //    }
            //}
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("HDO365HealthStatus", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<Hdo365HealthStatusLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case Hdo365HealthStatusLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id, cancellationToken: cancellationToken);
                            }
                        case Hdo365HealthStatusLuis.Intent.HealthCheck:
                            {
                                return await stepContext.BeginDialogAsync(_healthStatusDialog.Id, cancellationToken: cancellationToken);
                            }                       

                        case Hdo365HealthStatusLuis.Intent.None:
                        default:
                            {
        ...
        {
            public const string IncidentPrompt = "incidentPrompt";
        }
    }
}