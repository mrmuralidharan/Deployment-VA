...
namespace HDHelpSkill.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
            ...
            : base(nameof(MainDialog))
        {
            ...
            // Register dialogs
            _helpDialog = serviceProvider.GetService<HelpDialog>(); 
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["HDHelpSkill"].RecognizeAsync<HdHelpSkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("HDHelpSkill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<HdHelpSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case HdHelpSkillLuis.Intent.Help:
                            {
                                return await stepContext.BeginDialogAsync(_helpDialog.Id, cancellationToken: cancellationToken);
                            }

                        case HdHelpSkillLuis.Intent.None:
                        default:
                            {
