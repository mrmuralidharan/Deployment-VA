...
{
    public class TodayWeatherDialog : SkillDialogBase
    {
        private readonly IHttpContextAccessor _httpContext;
        private IConfiguration _configuration { get; set; }

        public TodayWeatherDialog(
            IServiceProvider serviceProvider,
            IHttpContextAccessor httpContext)
            : base(nameof(TodayWeatherDialog), serviceProvider)
        {
            ...
            var getCurrentWeatherDialog = new WaterfallStep[]
            {
                GetCurrentWeatherResponseAsync,
                EndAsync,
            };
            //AddDialog(new WaterfallDialog(nameof(TodayWeatherDialog), getCurrentWeatherResponseDialog));
            //AddDialog(new WaterfallDialog(DialogIds.GetCurrentWeatherDialog, getCurrentWeatherDialog));

            AddDialog(new WaterfallDialog(nameof(TodayWeatherDialog), getCurrentWeatherDialog));

            InitialDialogId = nameof(TodayWeatherDialog);
        }
        ...
        private async Task<DialogTurnResult> RouteToGeographyPromptOrForecastResponseAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                return await stepContext.ReplaceDialogAsync(DialogIds.GetCurrentWeatherDialog, cancellationToken: cancellationToken);
            }
        }

        /// <summary>
        /// Look up the current day weather using Accuweather.
        /// </summary>
        private async Task<DialogTurnResult> GetCurrentWeatherResponseAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (double.IsNaN(state.Latitude) && double.IsNaN(state.Longitude))
            {
                if (!string.IsNullOrEmpty(state.zipCode) && state.zipCode != state.Geography)
                {
                    ...
                    //    else
                    //    {
                    //        //List<SkillServiceLibrary.Models.AzureMaps.SearchResult> secondSearch = await service.GetCoordinatesByQueryAsyncForZipCode(state.zipCode);
                    //        //state.IsZipWithSpace = false;
                    ...
                    //int resIndex = qryRes.FindIndex(x => x.Address.PostalCode.Contains(state.zipCode));
                    //if (resIndex > 0)
                    //{
                    ...
                    //int resultIndex = res.FindIndex(x => x.Address.PostalCode.Contains(state.zipCode));
                    //if (resultIndex >= 0)
                    //{
            ...
            if (!double.IsNaN(state.Latitude) && !double.IsNaN(state.Longitude))
            {
                ...
                if (forecastModel.TempUnit == "°F")
                {
                    forecastModel.MinimumTemperature = (int)((oneDayForecast.Forecasts[0].Temperature.Minimum.Value * 1.8) + 32);
                    forecastModel.MaximumTemperature = (int)((oneDayForecast.Forecasts[0].Temperature.Maximum.Value * 1.8) + 32);
                }
        ...
        private static class DialogIds
        {
            ...
            public const string GetCurrentWeatherDialog = "getCurrentWeatherDialog";
        }
    }
}
