...
namespace SkillServiceLibrary.Services.AzureMapsAPI
{
    public sealed class AzureMapsGeoSpatialService : IGeoSpatialService
    {
        ...
        // convert to azure maps search categories
        ...
        public async Task<PointOfInterestModel> GetPointOfInterestDetailsAsync(PointOfInterestModel pointOfInterest, int width = 0, int height = 0)
        {
            ...
            Guard.NotNull(pointOfInterest, nameof(pointOfInterest));
            ...
            {
                return pointOfInterest; // Compliant, this code is not reachable
            }
        ...
        public async Task<string> GetRouteImageAsync(PointOfInterestModel destination, RouteDirections.Route route, int width = 0, int height = 0)
        {
            ...
            // we could use Data in Azure Maps Service to store the whole path
        ...
        private async Task<List<PointOfInterestModel>> GetPointsOfInterestAsync(string url, string poiType = null, string favoredEntity = null)
        {
            ...
            if (apiResponse?.Results != null)
            {
                ...
                if (!string.IsNullOrEmpty(poiType) && poiType == GeoSpatialServiceTypes.PoiType.Nearest)
                {
    ...
    {
        public static void NotNull<T>([AzureMapsGeoSpatialService.ValidatedNotNull] this T value, string name) where T : class
        {
            if (value == null)
