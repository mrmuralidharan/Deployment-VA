...
using AcronymLookup.Extensions;
...
namespace AcronymLookup.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly AcronymDialog _acronymDialog;        
            ...
            : base(nameof(MainDialog))
        {
            ...
            AddDialog(new WaterfallDialog(nameof(AcronymDialog), steps));
            AddDialog(new TextPrompt(nameof(TextPrompt)));
            InitialDialogId = nameof(MainDialog);

            // Register dialogs           
            _acronymDialog = serviceProvider.GetService<AcronymDialog>();          
            AddDialog(_acronymDialog);
        }
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["AcronymLookup"].RecognizeAsync<AcronymLookupLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("AcronymLookup", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<AcronymLookupLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {                        

                        case AcronymLookupLuis.Intent.AcronymLookup:
                            {
                                return await stepContext.BeginDialogAsync(_acronymDialog.Id, cancellationToken: cancellationToken);
                            }

                        case AcronymLookupLuis.Intent.None:
                        default:
                            {
